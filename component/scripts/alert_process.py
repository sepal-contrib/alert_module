...
def get_glad_alerts(aoi_model, model, alert):
    ...
    if not alert.check_input(model.start): return (None, None)
    if not alert.check_input(model.end): return (None, None)
    ...
    alerts_date = glad_import.get_alerts_dates(aoi_model.feature_collection, year, [start, end])
    download = drive_handler.download_to_disk(filename_date, alerts_date, aoi_model, alert)
...
def get_radd_alerts(aoi_model, model, alert):
    ...
    # check for the julian day task 
    filename_date = f'{filename}_dates'
    alerts_date = radd_import.get_alerts_dates(aoi_model.feature_collection, [start, end])
    download = drive_handler.download_to_disk(filename_date, alerts_date, aoi_model, alert)
...
def get_gee_assets(aoi_model, model, alert):
    ...
    if not alert.check_input(model.start): return (None, None)
    if not alert.check_input(model.end): return (None, None)
    if not alert.check_input(model.date_asset): return (None, None)
    if not alert.check_input(model.alert_asset): return (None, None)
    if not alert.check_input(model.asset_date_band): return (None, None)
    if not alert.check_input(model.asset_alerts_band): return (None, None)
    ...
    filename_date = f'{filename}_dates'
    alerts_date = gee_import.get_alerts_dates([start, end], aoi_model.feature_collection, model.date_asset, model.asset_date_band)
    download = drive_handler.download_to_disk(filename_date, alerts_date.unmask(0), aoi_model, alert)
    
    # reteive alert date masked with date range 
    filename_map = f'{filename}_map'
    alerts = gee_import.get_alerts(alerts_date, aoi_model.feature_collection, model.alert_asset, model.asset_alerts_band)
    download = drive_handler.download_to_disk(filename_map, alerts.unmask(0), aoi_model, alert)
...
def get_local_alerts(aoi_model, model, alert):
    ...
    if not alert.check_input(model.start): return (None, None)
    if not alert.check_input(model.start): return (None, None)
    if not alert.check_input(model.date_file): return (None, None)
    if not alert.check_input(model.alert_file): return (None, None)
    
    # check that the file exist 
    if not Path(model.date_file).is_file(): return (None, None)
    if not Path(model.alert_file).is_file(): return (None, None)
