...
def sepal_process(aoi_model, alert_model, alert):
    ...
    if not alert_date_map.is_file():
        cut_to_aoi(aoi_model, alert_date_tmp_map, alert_date_map)
    if not alert_map.is_file():
        cut_to_aoi(aoi_model, alert_tmp_map, alert_map)
    if not clump_map.is_file():
        cut_to_aoi(aoi_model, clump_tmp_map, clump_map)
...
def display_results(aoi_model, alert_model, alert, stats):
    ...
    m = display_alerts(
        aoi_model, result_dir / f"{basename}_map.tif", cp.color_palette, alert
    )
    ...
    # create the display
    children = [
        v.Layout(Row=True, children=[csv_btn, png_btn, tif_btn]),
        partial_layout,
    ]
...
def create_fig(df, title, alert_type):
    ...
    ax.hist(
        ...
        label=[*values],
        weights=y_,
        color=cp.color_palette[: len(values)],
        bins=30,
        histtype="bar",
        stacked=True,
        edgecolor="black",
...
def display_alerts(aoi_model, raster, colors, alert):
    ...
    try:
        ...
        for i in range(min_val, max_val + 1):
            if i == 3 or i == 1:
                ...
                color_map.append([0.0, 0.0, 0.0, 0.0])
    ...
    m.add_legend(
        legend_keys=legend_keys, legend_colors=colors[::-1], position="topleft"
    )
