...
class DriverTile(sw.Tile):
    def __init__(self, model, aoi_model):
        ...
        self.select_type = v.Select(
            items=cp.available_drivers, label=cm.driver.select_type, v_model=None
        )
        ...
        # local files file
        self.select_date_file = sw.FileInput(
            [".tif", ".tiff"], label=cm.driver.select_date_file
        ).hide()
        self.select_alerts_file = sw.FileInput(
            [".tif", ".tiff"], label=cm.driver.select_alerts_file
        ).hide()

        # date/alert asset
        self.select_date_asset = sw.AssetSelect(
            xs8=True, label=cm.driver.select_date_asset, _metadata={"band": "DATE"}
        )
        self.select_date_asset_band = v.Select(
            xs4=True, class_="pl-5", label="band", items=None, v_model=None
        )
        ...
        # alert asset
        self.select_alerts_asset = sw.AssetSelect(
            label=cm.driver.select_alerts_asset, _metadata={"band": "ALERT"}
        )
        self.select_alerts_asset_band = v.Select(
            xs4=True, class_="pl-5", label="band", items=None, v_model=None
        )
        ...
        self.model.bind(self.select_type, "alert_type").bind(
            self.start_picker, "start"
        ).bind(self.end_picker, "end").bind(self.select_date_file, "date_file").bind(
            self.select_alerts_file, "alert_file"
        ).bind(
            self.select_date_asset_band, "asset_date_band"
        ).bind(
            self.select_date_asset, "date_asset"
        ).bind(
            self.select_alerts_asset_band, "asset_alerts_band"
        ).bind(
            self.select_alerts_asset, "alert_asset"
        )
    ...
    def show_inputs(self, change):
        ...
        base_list = [
            self.picker_line
        ]  # the date pickers are used for every type of alerts

        if change["new"] == "GEE":  # gee assets
            fields_2_show = base_list + [
                self.gee_txt,
                self.asset_date_line,
                self.asset_alerts_line,
            ]
    ...
    def process_start(self, widget, event, data):
        ...
        self.model.date, self.model.alert = cs.get_alerts(
