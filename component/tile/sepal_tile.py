...
class SepalTile(sw.Tile):
    ...
    def _on_click(self, widget, event, data):
        ...
        # check inputs
        if not self.alert.check_input(self.aoi_model.name, cm.alert.no_aoi):
            return
        if not self.alert.check_input(self.model.alert, cm.alert.no_year):
            return
        ...
        if stats_link:
            ...
            layout = cs.display_results(
                self.aoi_model, self.model, self.alert, stats_link
            )
